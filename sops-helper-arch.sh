...
AGE_PUB_KEY_FILE="${AGE_KEY_DIR}/soul.pub" #TODO: Change this to a more unique name if you want to manage multiple keys
...
show_pubkey() {
    ...
    if [[ ! -f "$AGE_PUB_KEY_FILE" ]]; then
        log_error "No key found at $AGE_PUB_KEY_FILE"
        return 1
    fi
